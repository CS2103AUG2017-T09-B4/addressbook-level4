...
Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("favourite 2")` API call.
...
**Alternative 1 (current choice):** Create another class `FavouriteStatus` to hold data on whether the person is a favourite. +
**Pros:** Consistent with the other data fields like `Name` and `Email` +
**Cons:** Less memory efficient +
**Alternative 2:** Creating a boolean variable in `Person` class itself +
...
**Alternative 1 (current choice):** Use the existing `updatePerson(ReadOnlyPerson target, ReadOnlyPerson editedPerson)` method in `Model` +
...
**Alternative 2:** Implement additional methods in `Model`, `AddressBook`, etc. to specifically toggle a person's favourite status +
...
The remove feature allows users to remove a `Tag` either from a *single person* or from the *whole address book*.
The distinction in execution of the command lies in the format of the user command.

* Format: `remove friends` +
Removes the `friends` tag from the *whole address book*.

* Format: `remove friends 1` +
Removes the `friend` tag from *only the first person* in the address book.

The remove mechanism involves the `RemoveCommandParser`, which resides in the `LogicManager` to parse the user inputs.
The parsing mechanism in `RemoveCommandParser` uses the `StringTokenizer` class of the Java API to obtain the required
arguments to create a `RemoveCommand` object.

`RemoveCommand` extends `UndoableCommand`, thus, uses the `UndoRedoStack`, which resides in the `LogicManager`.
This allows any changes executed from the `RemoveCommand` to be undone, as specified by the `Undo/Redo` mechanism.

Execution of `RemoveCommand` is governed by the `removeTag` method located in the `Model` interface and implemented by
`ModelManager`.

==== Design Considerations
**Aspect:** Execution of remove command  +
**Alternative 1 (current choice):** Only accepts one `Tag` as parameter (Only removes one `Tag`). +
**Pros:** Complements the use of `Undo/Redo` command for removal of each `Tag`. +
**Cons:** May be an inconvenience to user who wants to remove multiple `Tags`. +
**Alternative 2:** Allow multiple `Tags` as parameters (Removes multiple `Tags`). +
**Pros:** Better efficiency in mass removal of `Tags` from a person or the address book. +
**Cons:** Users may face inconvenience in undoing removal of a *single* `Tag` after the removal of *multiple* `Tags`. +

---

**Aspect:** Execution of remove command for specified user. +
**Alternative 1 (current choice):** Only accepts one `Index` as parameter. +
**Pros:** Allows further user-customisations by removing `Tag` from intended person. +
**Cons:** Inconvenience for users to remove specified `Tag` from multiple persons. +
**Alternative 2:** Allow for multiple `Index` to be accepted as parameters. +
**Pros:** More convenient for users to remove a `Tag` from multiple users. +
**Cons:** Current implementation updates the address book immediately after each removal of `Tag`. This alternative may
result in errors or inaccurate execution of command. +
...
To help with this feature, we planned to use an ArrayList to store the remarks under each person, as it would help with indexing the remarks for easy removal and additions. +
As ArrayLists naturally update the index of the its list, we do not need additional functions to handle indexing. +
On top of this, we decided that remarks should be updated with any changes, either to selection or file storage. +
...
|`* *` |user |undo/redo amendments made to the address book |immediately revert to previous version if mistakes are made
...
|`*` |user |view recent contacts added/searched |keep a log of the operations done in the address book
...
|`*` |user| sort certain persons by where they stay| know where is a convenient meetup location
...
4.  AddressBook sets the person as a favourite contact
+
Use case ends.
...
4.  AddressBook adds the person into the group
+
Use case ends.
...
2.  AddressBook confirms the request, yes or no
3.  User enters password
4.  AddressBook requests for password again to confirm
5.  User enters password again
6.  AddressBook sets up password and stores data in a .xml file
7.  AddressBook requests for answer to a predetermined security question.
8.  User enters answer
9.  AddressBook stores security answer in a .xml file linked to the account
10.  User needs to type in login to get his private addressbook, else it will show a public addressbook
+
Use case ends.
...
+
** 10a1. AddressBook reveals first and last letter of password.
+
** 10a2. User has forgotten the password
+
** 10a3. AddressBook prompts user to provide security answer
+
** 10a4. AddressBook check if it match the answer from .xml file
+
** 10a4. AddressBook reveals the password if correct
+
...
4.  AddressBook shows restricted list
5.  User requests to sort persons by address
6.  AddressBook shows a list of persons sorted by address
+
Use case ends.
...
4.  AddressBook edits specified contact's details
5.  AddressBook shows amended contact's details
+
Use case ends.
...
4.  AddressBook shows message for successful deletion
+
Use case ends.
...
2.  AddressBook shows a list of persons with fields containing given input
+
Use case ends.
...
....
The text bar where user types in when AddressBook is opened up
....

[[fields]]

....
The different component of contact's details.
....
